...
public class Classification {
	...
	static {
		...
		addSet(mediaExtSet, apps + "," + audio + "," + video + "," + image); // all
																				// media
																				// formats
	}
	...
	public static void init(final File mimeFile) {
		...
		for (Entry<Object, Object> entry : mimeTable.entrySet()) {
			...
			if (mime.startsWith("text/"))
				textExtSet.add(ext.toLowerCase());
			if (mime.startsWith("audio/"))
				audioExtSet.add(ext.toLowerCase());
			if (mime.startsWith("video/"))
				videoExtSet.add(ext.toLowerCase());
			if (mime.startsWith("application/"))
				appsExtSet.add(ext.toLowerCase());
		}
	}
	 ...
	 * This can produce wrong results because the extension is a weak hint for
	 * the content domain. If possible, use the mime type, call
	 * Classification.getContentDomainFromMime()
	 * 
	...
	public static ContentDomain getContentDomainFromExt(final String ext) {
		...
		if (isApplicationExtension(ext))
			return ContentDomain.APP;
		if (isControlExtension(ext))
	...
	public static String ext2mime(final String ext) {
		return ext == null ? "application/octet-stream"
				: mimeTable.getProperty(ext.toLowerCase(),
						"application/" + (ext == null || ext.length() == 0 ? "octet-stream" : ext));
	}
