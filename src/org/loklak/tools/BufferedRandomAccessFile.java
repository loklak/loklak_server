 ...
 * https://code.google.com/p/jmzreader/source/browse/tools/braf/trunk/src/main/
 * java/uk/ac/ebi/pride/tools/braf/BufferedRandomAccessFile.java which is in
 * turn an optimized version of the RandomAccessFile class as described by Nick
 * Zhang on JavaWorld.com. The article can be found at
 * http://www.javaworld.com/javaworld/javatips/jw-javatip26.html The getNextLine
 * method was rewritten in such a way that it returns a byte[] rather than a
 * string which was not UTF-8 - friendly in it's original version. The whole
 * class was not concurrency-safe. Synchronization has been added to ensure
...
public class BufferedRandomAccessFile extends RandomAccessFile {
	...
	/**
	 * Overridden seek method always throws exception: this would not work in
	 * concurrent environments. All seek operations must be encapsulated here in
	...
	public synchronized long appendLine(final byte[] b) throws IOException {
		...
		this.write(b);
		this.writeByte((byte) '\n');
		this.invalidate(); // instead of invalidate it could be better to
							// refresh the buffer with the latest byte[]
		return seekpos;
	}

	/**
	 * Reading of text lines will produce index information along with the
	 * parsed text. To get the exact number of bytes, we do not depend on a
	 * utf-8 - parsing string but instead the line is read as byte[] to
	 * determine the exact length of the line.
	 * 
	...
	public static class Test extends TestCase {
		...
		public static String[] getTestLines(int count) {
			...
			for (int i = 0; i < count; i++)
				lines[i] = "{\"" + Long.toString(r.nextLong()) + "\":\"X\"}";
			return lines;
		}
		...
		public void testSimultanousWriteAndRead() throws IOException {
			...
			for (int i = 0; i < this.testLines.length; i++) {
				...
				if (!ASCII.String(b).equals(this.testLines[i]))
					System.out.println(ASCII.String(b) + " != " + this.testLines[i]);
