...
public final class ByteBuffer extends OutputStream {
	...
	public void clear() {
		// we keep the byte[] and just set the pointer to write positions to
		// zero
		this.length = 0;
		this.offset = 0;
	}
	...
	public ByteBuffer trim(final int start, final int len) {
		if (start + len > this.length)
			throw new IndexOutOfBoundsException("trim: start + len > length; this.offset = " + this.offset
					+ ", this.length = " + this.length + ", start = " + start + ", len = " + len);
	...
	public static boolean equals(final byte[] buffer, final byte[] pattern) {
		// compares two byte arrays: true, if pattern appears completely at
		// offset position
