...
public class Browser {
	...
	private static void openBrowserMac(final String url) throws Exception {
		Process p = Runtime.getRuntime()
				.exec(new String[] { "/usr/bin/osascript", "-e", "open location \"" + url + "\"" });
		p.waitFor();
		if (p.exitValue() != 0) {
	...
	/*
	 * private static void openBrowserUnixGeneric(final String url) throws
	 * Exception { String cmd = "/etc/alternatives/www-browser " + url; Process
	 * p = Runtime.getRuntime().exec(cmd); p.waitFor(); if (p.exitValue() != 0)
	 * throw new RuntimeException("Unix Exec Error/generic: " +
	 * errorResponse(p)); }
	 */
	private static void openBrowserWin(final String url) throws Exception {
		// see forum at
		// http://forum.java.sun.com/thread.jsp?forum=57&thread=233364&message=838441
		String cmd;
