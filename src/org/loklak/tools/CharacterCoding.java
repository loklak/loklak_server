...
public final class CharacterCoding {
	...
	private static final String[] MAPPING4XML = { "\"", "&quot;", // quotation
																	// mark
			"\u003C", "&lt;", // less than
			"\u003E", "&gt;", // greater than
	};
	...
	private static final String[] MAPPING4HTML = { "\\", "&#092;", // Backslash
			"\u005E", "&#094;", // Caret
	...
	private static final Map<String, Character> HTML2UNICODE4XML = new HashMap<String, Character>(
			MAPPING4XML.length / 2);
	/** Mapping for HTML to unicode. */
	private static final Map<String, Character> HTML2UNICODE4HTML = new HashMap<String, Character>(
			MAPPING4HTML.length / 2);
	/** Mapping for unicode to XML. */
	private static final Map<Character, String> UNICODE2HTML4XML = new HashMap<Character, String>(
			MAPPING4XML.length / 2);
	/** Mapping for unicode to HTML. */
	private static final Map<Character, String> UNICODE2HTML4HTML = new HashMap<Character, String>(
			MAPPING4HTML.length / 2);
	static {
		Character c;
	...
	/**
	 * Private constructor to avoid instantiation of utility class with only
	 * static methods.
	 */
	 ...
	 * @see #MAPPING4XML
	 * @param text
	 *            text with character to replace
	 * @param amp
	 *            true if ampersands shall be replaced, else false
	 ...
	 *            true if ampersands shall be replaced, else false
	 * @param html
	 *            true if characters shall be replaced for embedding in HTML,
	 *            false for XML (far more characters are replaced for HTML,
	...
	public static String html2unicode(String text) {
		if (text == null)
			return "";
		text = CommonPattern.AMP_HTML.matcher(text).replaceAll("&"); // sometimes
																		// a
																		// double-replacement
																		// is
																		// necessary.
		...
		while (p < text.length()) {
			...
			q = text.indexOf(';', p);
			if (q < 0) {
				// if there is now no semicolon, then this will also fail when
				// another ampersand is found afterwards
