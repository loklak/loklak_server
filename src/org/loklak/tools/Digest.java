...
public class Digest {
	...
	public static byte[] decodeHex(final String hex) {
		...
		for (int i = 0; i < result.length; i++) {
			result[i] = (byte) (16 * Integer.parseInt(Character.toString(hex.charAt(i * 2)), 16)
					+ Integer.parseInt(Character.toString(hex.charAt(i * 2 + 1)), 16));
		}
		return result;
	}
	...
	public static byte[] encodeMD5Raw(final String key) {
		...
		if (digest == null) {
			...
			// this is not the most effective way but if we wouldn't do that the
			// encoder would block
			try {
			...
			digest.reset(); // they should all be reseted but anyway; this is
							// safe
		}
		byte[] keyBytes;
		...
		// update the cache
		md5Cache.put(key, result); // prevent expensive MD5 computation and
									// encoding
		return result;
	}
	...
	/**
	 * the fast fingerprint computes a md5-like hash from a given file, which is
	 * different from a md5 because it does not read the complete file but reads
	 * only the first and last megabyte of it. In case that the file is less or
	 * equal of one megabyte, the fast fingerprint is equal to the md5. In other
	 * cases the fingerprint is computed from a array = byte[32k + 8] array,
	 * which consists of: array[0 .. 16k - 1] = first MB of file array[16k ..
	 * 32k - 1] = last MB of file array[32k .. 32k + 7] = length of file as long
	 * if the date flag is set, the array is extended to array[32k + 8 .. 32k +
	 * 15] = lastModified of file as long
	 * 
	 * @param file
	...
	public static byte[] fastFingerprintRaw(final File file, final boolean includeDate) throws IOException {
		...
		try {
			...
			if (includeDate)
				digest.update(longToBytes(file.lastModified()), 0, 8);
		} finally {
			raf.close();
			try {
