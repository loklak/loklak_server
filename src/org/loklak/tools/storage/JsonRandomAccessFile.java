...
public class JsonRandomAccessFile extends BufferedRandomAccessFile implements JsonReader {
	...
	/**
	 * if a JsonRandomAccessFile object in initiated, it must be wrapped with a
	 * Thread object and started.
	 * 
	 * @param dumpFile
	 * @param concurrency
	 * @throws IOException
		...
		try {
			...
			while ((line = this.readIndexedLine()) != null) {
				try {
					...
					Log.getLog().warn(
							"cannot parse line in file " + JsonRandomAccessFile.this.file + ": \"" + line + "\"", e);
				}
			}
		} catch (IOException e) {
			Log.getLog().warn(e);
	 ...
	 * seek location in the file and the length of bytes of the original json
	 * string
	 */
