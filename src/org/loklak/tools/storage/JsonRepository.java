...
public class JsonRepository {
	...
	public static enum Mode {
		COMPRESSED, // dump files are compressed but cannot be re-written. All
					// data is cached in RAM.
		REWRITABLE; // dump files are not compressed but can be re-written. Data
					// is only indexed in RAM and retrieved from file.
	}
	...
	public JsonRepository(File dump_dir, String dump_file_prefix, String readme, final Mode mode,
		...
		this.json_log = new JsonRandomAccessFile(getCurrentDump(dump_dir_own, this.dump_file_prefix, mode, dailyDump),
				this.concurrency);
	...
	private static File getCurrentDump(File path, String prefix, final Mode mode, final boolean dailyDump) {
		...
		if (existingDumps != null) {
			for (String d : existingDumps) {
				// first check if the file is the current file: we never
				// compress that to enable a write to the end of the file
				if (d.startsWith(prefix + currentDatePart) && d.endsWith(".txt")) {
					continue;
				}

				// according to the write mode, we either compress or uncompress
				// the file on-the-fly
				if (mode == COMPRESSED_MODE) {
					// all files should be compressed to enable small file
					// sizes, but contents must be in RAM after reading
					...
					// all files should be uncompressed to enable random-access
					// mode
			...
			// the latest file with the current date is the required one (and it
		...
		// The random is used to make it possible to join many different dumps
		// from different instances
		// without renaming them
		String random = (Long.toString(Math.abs(new Random(System.currentTimeMillis()).nextLong())) + "00000000")
				.substring(0, 8);
		return new File(path, prefix + currentDatePart + "_" + random + ".txt");
	}

	public JsonFactory write(JSONObject json) throws IOException {
		String line = json.toString(); // new
										// ObjectMapper().writer().writeValueAsString(map);
		JsonFactory jf = null;
	...
	public JsonFactory write(JSONObject json, char opkey) throws IOException {
		...
		sb.append('{').append('\"').append(UTF8.String(OPERATION_KEY)).append('\"').append(':').append('\"')
				.append(opkey).append('\"').append(',');
	...
	private static SortedSet<File> getDumps(final File path, final String prefix, final String suffix, int count) {
		...
		TreeSet<File> dumps = new TreeSet<File>(); // sort the names with a tree
													// set
		for (String s : list) {
	 ...
	 * 
	 * @param dumpName
	 *            only the name, not the full path. The file must be in the
	 *            import file path
	 * @return true if the file was shifted successfully, false if file did not
	 *         exist or cannot be moved
	 */
	...
	/**
	 * create a concurrent dump reader for the given file. The reader is either
	 * a JsonStreamReader if the dump file is gzipped or a JsonRandomAccessFile
	 * if the file is a plain txt file. Both reader types must be started as
	 * concurrent process which this method does on it's own. The reader process
	 * dies automatically when the file is read completely. When the reader
	 * thread dies, it pushed several JsonReader.POISON_JSON_MAP objects to the
	 * reading queue, according to the concurrency defined with the initializer
	 * of this class.
	 * 
	 * @param dump
	 *            file
