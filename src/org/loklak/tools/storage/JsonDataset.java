...
public class JsonDataset {
	...
	private final JsonRepository indexDump; // a directory containing dump,
											// import and imported
											// subdirectories
	final Map<String, JsonFactoryIndex> index; // a mapping from a search key to
												// the search index
	private final JsonMinifier minifier; // a minifier for json which learns
											// about json mapping key names
	private final Map<String, Boolean> columns; // a mapping from the column key
												// to a boolean which is true if
												// the column value is
												// case-insensitive
	private final String dateFieldName; // a name of a date field which shows
	 ...
	 * 
	 * @param dump_dir
	 *            the path where the subdirectories for this data set shall be
	 *            stored
	 * @param dump_file_prefix
	 *            a prefix for the file names
	 * @param index_keys
	 *            the names of the json property keys where their content shall
	 *            be indexed by this field
	 * @param mode
	 *            the indexing mode, either completely in RAM with
	 *            Mode.COMPRESSED or with file handles with Mode.REWRITABLE
	 * @throws IOException
	 */
	public JsonDataset(File dump_dir, String dump_file_prefix, Column[] columns, String dateFieldName,
			String dateFieldFormat, JsonRepository.Mode mode, final boolean dailyDump, int count) throws IOException {
		...
		// for each reader one threqd is started which does Json parsing and
		// indexing
		if (dumps != null)
							...
							try {
								while ((jsonHandle = reader.take()) != JsonStreamReader.POISON_JSON_MAP) {
									...
									if (jsonHandle instanceof JsonRandomAccessFile.JsonHandle) {
										...
										// create the file json handle which
										// does not contain the json any more
										// but only the file handle
										jsonFactory = ((JsonRandomAccessFile) reader).getJsonFactory(handle.getIndex(),
												handle.getLength());
									} else {
										assert JsonDataset.this.indexDump.getMode() == JsonRepository.COMPRESSED_MODE;
										// create the json minifier object which
										// contains the json in minified version
										// before we create the minifier, we
										// remove the meta keys from the json to
										// further minify it
									...
									// the resulting json factory is written to
									// each search index
									for (Map.Entry<String, Boolean> column : JsonDataset.this.columns.entrySet()) {
										...
										if (searchValue != null) {
											if (searchValue instanceof String) {
												JsonFactory old = factoryIndex.put(case_insensitive
														? ((String) searchValue).toLowerCase() : (String) searchValue,
														jsonFactory);
											} else {
	...
	public JsonFactory putUnique(JSONObject obj) throws IOException {
		...
		for (Map.Entry<String, Boolean> column : this.columns.entrySet()) {
			// for (Map.Entry<String, JsonFactoryIndex> idxo:
			// this.index.entrySet()) {
			String searchKey = column.getKey();
			boolean case_insensitive = column.getValue();
	...
	public JsonFactory get(String column, String value) {
		...
		if (insensitive == null)
			throw new RuntimeException("Column " + column + " was not declared");
		JsonFactoryIndex jfi = this.index.get(column);
		if (jfi == null)
			throw new RuntimeException("Column " + column + " was not defined");
	...
	public Date parseDate(JSONObject json) throws ParseException {
		...
		if (!(d instanceof String))
			throw new ParseException("date field in json must contain a String or Date, not " + d.getClass().getName(),
					0);
	...
	public static class JsonFactoryIndex extends ConcurrentHashMap<Object, JsonFactory>
			implements Map<Object, JsonFactory> {
