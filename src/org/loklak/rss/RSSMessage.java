...
public class RSSMessage {
	 ...
	 */
	// private static final SimpleDateFormat FORMAT_ISO8601 = new
	// SimpleDateFormat(PATTERN_ISO8601, Locale.US);

	/**
	 * Date formatter/parser for standard compliant HTTP header dates (RFC 1123)
	 */
	private static final String PATTERN_RFC1123 = "EEE, dd MMM yyyy HH:mm:ss Z"; // with
																					// numeric
																					// time
																					// zone
																					// indicator
	...
	private static final SimpleDateFormat SHORT_SECOND_FORMATTER = new SimpleDateFormat(PATTERN_SHORT_SECOND,
			Locale.US);
	...
	public static enum Token {
		...
		title(new String[] { "title", "atom:title", "rss:title" }), link(
				new String[] { "link", "atom:link", "rss:link" }), description(new String[] { "description", "subtitle",
						"atom:subtitle", "rss:description" }), pubDate(new String[] { "pubDate", "lastBuildDate",
								"updated", "rss:lastBuildDate", "rss:updated" }), copyright(
										new String[] { "copyright", "publisher" }), author(new String[] { "author",
												"creator" }), subject(new String[] { "subject" }), category(
														new String[] { "category" }), referrer(new String[] {
																"referrer", "referer" }), language(new String[] {
																		"language" }), guid(new String[] {
																				"guid" }), ttl(new String[] {
																						"ttl" }), docs(new String[] {
																								"docs" }), size(
																										new String[] {
	...
	public RSSMessage(final String title, final String description, final String link) {
		...
		this.map.put(Token.guid.name(),
				artificialGuidPrefix + Integer.toHexString((title + description + link).hashCode()));
	}
	...
	public String getGuid() {
		...
		if ((guid.isEmpty() || guid.startsWith(artificialGuidPrefix)) && (this.map.containsKey("title")
				|| this.map.containsKey("description") || this.map.containsKey("link"))) {
			guid = calculatedGuidPrefix
					+ Integer.toHexString(getTitle().hashCode() + getDescriptions().hashCode() + getLink().hashCode());
	...
	public String getFulltext() {
		...
		for (final String s : this.map.values())
			sb.append(s).append(' ');
		return sb.toString();
	}
	...
	public String toString(boolean withItemTag) {
		...
		if (this.map.containsKey(Token.title.name()))
			sb.append("<title>").append(CharacterCoding.unicode2xml(this.map.get(Token.title.name()), true))
					.append("</title>\n");
		if (this.map.containsKey(Token.link.name()))
			sb.append("<link>").append(CharacterCoding.unicode2xml(this.map.get(Token.link.name()), true))
					.append("</link>\n");
		if (this.map.containsKey(Token.description.name()))
			sb.append("<description>").append(CharacterCoding.unicode2xml(this.map.get(Token.description.name()), true))
					.append("</description>\n");
