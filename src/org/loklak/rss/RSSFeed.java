...
public class RSSFeed implements Iterable<RSSMessage> {
	...
	/**
	 * make a RSS feed using a set of urls the source string is assigned to all
	 * messages as author to mark the messages' origin
	 * 
	 * @param links
	 * @param source
	 */
	...
	public Set<URL> getLinks() {
		...
		for (RSSMessage message : this.messages.values()) {
			try {
				links.add(new URL(message.getLink()));
	...
	public void addMessage(final RSSMessage item) {
		...
		// in case that the feed is full (size > maxsize) flush the oldest
		// element
	...
	public RSSMessage pollMessage() {
		...
		synchronized (this.messages) {
			if (this.messages.isEmpty())
				return null;
			final String nextGUID = this.messages.keySet().size() == 0 ? null
					: this.messages.keySet().iterator().next();
			if (nextGUID == null)
				return null;
	...
	public class messageIterator implements Iterator<RSSMessage> {
		...
		public boolean hasNext() {
			if (this.t <= 0)
				return false; // ensure termination
