...
public class PushServletHelper {
	...
	public static final String[] FIELDS_TO_COMPARE = { "link", "text" // can
																		// embed
																		// rich
																		// content
	};

	public static PushReport saveMessagesAndImportProfile(JSONArray messages, int fileHash, Query post,
		...
		if (report.getNewCount() > 0) {
			ImportProfileEntry importProfileEntry = saveImportProfile(fileHash, post, sourceType, screenName,
					importedMsgIds);
	...
	protected static ImportProfileEntry saveImportProfile(int fileHash, Query post, SourceType sourceType,
			String screenName, List<String> importedMsgIds) throws IOException {
		...
		if (!"".equals(harvesting_freq)) {
			try {
				profile.put("harvesting_freq",
						HarvestingFrequency.valueOf(Integer.parseInt(harvesting_freq)).getFrequency());
	...
	public static String buildJSONResponse(String callback, PushReport pushReport) throws IOException {
		...
		if (importProfile != null)
			json.put("importProfile", importProfile.toJSON());
