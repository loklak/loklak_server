...
public class GeoJsonPushServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		PushReport report = PushServletHelper.saveMessagesAndImportProfile(rawMessages, Arrays.hashCode(jsonText), post,
				sourceType, screen_name);
		...
		DAO.log(request.getServletPath() + " -> records = " + report.getRecordCount() + ", new = "
				+ report.getNewCount() + ", known = " + report.getKnownCount() + ", error = " + report.getErrorCount()
				+ ", from host hash " + remoteHash);
	}

	/**
	 * For each member m in properties, if it exists in mapRules, perform these
	 * conversions : - m:c -> keep value, change key m to c - m:c.d ->
	 * insert/update json object of key c with a value {d : value}
	 * 
	 * @param mapRules
	 * @param properties
