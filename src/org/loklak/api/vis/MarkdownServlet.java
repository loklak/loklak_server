...
public class MarkdownServlet extends HttpServlet {
	...
	protected void process(HttpServletRequest request, HttpServletResponse response, Query post)
		...
		long color_text = Long.parseLong(post.get("color_text", drawmode == DrawMode.MODE_SUB ? "000000" : "ffffff"),
				16);
		long color_code = Long.parseLong(post.get("color_code", drawmode == DrawMode.MODE_SUB ? "66ffaa" : "00ff00"),
				16);
		// long color_bold = Long.parseLong(post.get("color_bold", drawmode ==
		// DrawMode.MODE_SUB ? "ff66aa" : "ff0000"), 16);
		long color_headline = Long
				.parseLong(post.get("color_headline", drawmode == DrawMode.MODE_SUB ? "44aaee" : "00aaff"), 16);
		if (drawmode == DrawMode.MODE_SUB)
			color_text = RasterPlotter.invertColor(color_text);
		...
		while (lineheight <= 12) {
			...
			// a perfect size has the format 2:1, that fits into the preview
			// window.
			// We should not allow that the left or right border is cut away;
			// resize if necessary
			if (width <= 2 * height)
				break;
		...
		for (int pos = 0; pos < sb.length(); pos++) {
			...
			if (!isFormatted && !isInlineFormatted && column == 0 && c == '#') {
				...
				hashcount = Math.min(6, hashcount + 1); // there may be at most
														// 6
			...
			if (!isFormatted && !isInlineFormatted && c == '*' && pos > 0 && sb.charAt(pos - 1) == '*'
					&& pos < sb.length() - 1 && sb.charAt(pos + 1) != ' ') {
				isBold = true;
				continue;
			}
			if (!isFormatted && !isInlineFormatted && c == '*' && pos > 0 && sb.charAt(pos + 1) == '*' && pos > 0
					&& sb.charAt(pos - 1) != ' ') {
				isBold = false;
				continue;
			}
			...
			if (!isFormatted && c == '`' && pos < sb.length() - 2 && sb.charAt(pos + 1) == '`'
					&& sb.charAt(pos + 2) == '`') {
			...
			PrintTool.print(matrix, x, y, 0, uppercase || hashcount > 0 ? Character.toUpperCase(c) : c, isItalic,
					default_intensity);
			if (isBold)
				PrintTool.print(matrix, x + 1, y, 0, uppercase || hashcount > 0 ? Character.toUpperCase(c) : c,
						isItalic, default_intensity);
			x += 6;
			column++;
		}

		// write branding
		PrintTool.print(matrix, matrix.getWidth() - 6, matrix.getHeight() - 6, 0, "MADE WITH HTTP://LOKLAK.NET", 1,
				false, 50);
