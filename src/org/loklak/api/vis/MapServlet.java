...
public class MapServlet extends HttpServlet {
	...
	protected void process(HttpServletRequest request, HttpServletResponse response, Query post)
		...
		if (zoom > 19) {
			response.sendError(503, "zoom must be <= 19");
			return;
		}
		double lat = post.get("mlat", 50.11362d);
		if (lat < -90.0) {
			response.sendError(503, "mlat must be >= -90.0");
			return;
		}
		if (lat > 90.0) {
			response.sendError(503, "mlat must be <= 90.0");
		...
		if (lon < -180.0) {
			response.sendError(503, "mlon must be >= -180.0");
			return;
		}
		if (lon > 180.0) {
			response.sendError(503, "mlon must be <= 180.0");
		...
		if (width > 2560) {
			response.sendError(503, "width must be <= -2560");
			return;
		}
		int height = post.get("height", 256);
		if (height > 2560) {
			response.sendError(503, "height must be <= -2560");
			return;
		}
		int tiles_horizontal = (width - 1) / 256 + 1;
		if (((tiles_horizontal / 2) * 2) == tiles_horizontal)
			tiles_horizontal++;
		int tiles_vertical = (height - 1) / 256 + 1;
		if (((tiles_vertical / 2) * 2) == tiles_vertical)
			tiles_vertical++;
		...
		if (text.length() > 0)
			PrintTool.print(map, 6, 12, 0, uppercase ? text.toUpperCase() : text, -1, false, 100);
		...
		final BufferedImage logo = ImageIO
				.read(FileSystems.getDefault().getPath("html").resolve("artwork").resolve("marker-red.png").toFile());
		map.insertBitmap(logo, Math.min(map.getWidth() - 25, Math.max(0, mx - 12)),
				Math.min(map.getHeight() - 40, Math.max(0, my - 40)), FilterMode.FILTER_ANTIALIASING);

		// draw copyright notice on map
		/*
		 * copyright notice on OSM Tiles According to
		 * http://www.openstreetmap.org/copyright/ the (C) of the map tiles is
		 * (CC BY-SA) while the OpenStreetMap raw data is licensed with (ODbL)
		 * http://opendatacommons.org/licenses/odbl/ Map tiles shall be
		 * underlined with the statement "(C) OpenStreetMap contributors". In
		 * our 5-dot character set the lowercase letters do not look good, so we
		 * use uppercase only. The (C) symbol is not available in our font, so
		 * we use the letters (C) instead.
		 */
