...
/**
 * The search servlet. we provide opensearch/rss and twitter-like JSON as
 * result.
 */
public class SearchServlet extends HttpServlet {
	...
	public static Timeline search(final String protocolhostportstub, final String query, final Timeline.Order order,
			final String source, final int count, final int timezoneOffset, final String provider_hash,
		...
		try {
			...
			if (jsonb == null || jsonb.length == 0)
				throw new IOException("empty content from " + protocolhostportstub);
	...
	protected void doGet(final HttpServletRequest request, final HttpServletResponse response)
		...
		try {
			...
			String source = post.isDoS_servicereduction() ? "cache" : post.get("source", "all"); // possible
																									// values:
																									// cache,
																									// backend,
																									// twitter,
			...
			if (query.indexOf("id:") >= 0 && ("all".equals(source) || "twitter".equals(source)))
				source = "cache"; // id's cannot be retrieved from twitter with
									// the scrape-api (yet), only from the cache
			...
			if ("all".equals(source)) {
				...
				Thread scraperThread = tokens.raw.length() == 0 ? null : new Thread() {
					public void run() {
						...
						tl.putAll(QueryEntry.applyConstraint(twitterTl, tokens, false)); // pre-localized
																							// results
																							// are
																							// not
																							// filtered
				...
				// start a backend search, but only if backend_push == true or
				// result from scraper is too bad
				boolean start_backend_thread = false;
				if (backend_push)
					start_backend_thread = true;
				else {
					// wait now for termination of scraper thread and local
					// search
				...
				Thread backendThread = tokens.original.length() == 0 || !start_backend_thread ? null : new Thread() {
					public void run() {
						...
						if (backendTl != null) {
							...
							// TODO: read and aggregate aggregations from
							// backend as well
						}
				...
				tl.setScraperInfo(twitterTl.getScraperInfo());
				post.recordEvent("twitterscraper_time", System.currentTimeMillis() - start);
				// in this case we use all tweets, not only the latest one
				// because it may happen that there are no new and that is not
				// what the user expects

			} else if ("cache".equals(source)) {
				final long start = System.currentTimeMillis();
				DAO.SearchLocalMessages localSearchResult = new DAO.SearchLocalMessages(query, order, timezoneOffset,
						last_cache_search_time.get() > SEARCH_CACHE_THREASHOLD_TIME ? SEARCH_LOW_COUNT : count,
				...
				if (backendTl != null) {
					...
					// TODO: read and aggregate aggregations from backend as
					// well
					count_backend.set(tl.size());
				}
				post.recordEvent("backend_time", System.currentTimeMillis() - start);
			...
			if (jsonExt) {
				...
				try {
					...
					// late incoming messages from concurrent peer retrieval may
					// cause this
					// we silently do nothing here and return what we listed so
					// far
				}
	...
	public static void main(String[] args) {
		try {
			Timeline tl = search("http://loklak.org", "beer", Timeline.Order.CREATED_AT, "cache", 20, -120,
					backend_hash, 10000);
