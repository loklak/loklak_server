 ...
 * http://localhost:9000/api/console.json?q=SELECT%20*%20FROM%20timeanddate%20WHERE%20url=%27http://www.timeanddate.com/%27;
 * */
...
public class ConsoleService extends AbstractAPIHandler implements APIHandler {
    ...
    static {
        ...
        dbAccess.put(Pattern.compile("SELECT\\h+?(.*?)\\h+?FROM\\h+?timeanddate\\h+?WHERE\\h+?url\\h??=\\h??'(.*?)'\\h??;"), (flow, matcher) -> {
            SusiThought json = TimeAndDateService.timeAndDate(matcher.group(2));
            SusiTransfer transfer = new SusiTransfer(matcher.group(1));
            json.setData(transfer.conclude(json.getData()));
            return json;
        });
