...
public class ThreaddumpServlet extends HttpServlet {
	...
	private static final Thread.State[] ORDERED_STATES = new Thread.State[] { Thread.State.BLOCKED,
			Thread.State.RUNNABLE, Thread.State.TIMED_WAITING, Thread.State.WAITING, Thread.State.NEW,
	...
	protected void doGet(final HttpServletRequest request, final HttpServletResponse response)
		...
		if (servlet.length() > 0) {
			try {
				...
				final Method getMethod = servletClass.getDeclaredMethod("doGet", HttpServletRequest.class,
		...
		bufferappend(buffer, keylen, "Available  Memory",
		...
		int timetorestartminutes = (int) (timetorestartseconds / 60);
		timetorestartseconds = timetorestartseconds % 60;
		int timetorestarthours = timetorestartminutes / 60;
		timetorestartminutes = timetorestartminutes % 60;
		bufferappend(buffer, keylen, "Time To Restart",
				timetorestarthours + "h " + timetorestartminutes + "m " + timetorestartseconds + "s");
		// print system beans
	...
	private static class ThreadDump extends HashMap<StackTrace, SortedSet<String>>
			implements Map<StackTrace, SortedSet<String>> {
		...
		private ThreadDump(final Map<Thread, StackTraceElement[]> stackTraces, final Thread.State stateIn) {
			super();
			...
			for (final Map.Entry<Thread, StackTraceElement[]> entry : stackTraces.entrySet()) {
				...
				if (threadState != null && info != null && (stateIn == null || stateIn.equals(threadState))
					...
					final String threadtitle = tracename + "THREAD: " + thread.getName() + " "
							...
							+ (info.getLockOwnerId() >= 0 ? " lock owner =" + info.getLockOwnerId() : "");
		...
		private void appendStackTraces(final StringBuilder buffer, final Thread.State stateIn) {
