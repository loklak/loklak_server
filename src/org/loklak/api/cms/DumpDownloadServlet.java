	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (path.length() <= 1) {
			...
			for (File dump : DAO.getTweetOwnDumps(limited_count)) {
				...
				String size = length < 1024 ? Long.toString(length)
						: length < 1024 * 1024 ? Long.toString(length / 1024) + "K"
				...
				writer.write(" <a href=\"" + name + "\">" + name + "</a>" + space
						+ new Date(dump.lastModified()).toString() + "  " + size + "\n");
			}
			if (limited_count != Integer.MAX_VALUE)
				writer.write(limited_message + "\n");
			writer.write("<hr></pre>\n");
			writer.write("<address>this is the download directory for dumps of the message index</address>\n");
			writer.write(
					"<address>- import these dumps by placing them into your data/dump/import/ directory</address>\n");
