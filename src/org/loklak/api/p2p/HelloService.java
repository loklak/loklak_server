...
/**
 * Servlet to span the message peer-to-peer network. This servlet is called to
 * announce the existence of the remote peer.
 */
public class HelloService extends AbstractAPIHandler implements APIHandler {
	...
	public static void propagate(final String[] hoststubs) {
		...
		LinkedHashMap<String, Long> fullDateHistogram = DAO.FullDateHistogram(timezoneOffset); // complex
																								// operation
																								// can
																								// take
																								// some
		...
		for (String hoststub : hoststubs) {
			...
			try {
				String urlstring = hoststub + "/api/hello.json?port.http=" + httpport + "&port.https=" + httpsport
						...
						+ peakDay + "&peakCount=" + peakCount + "&firstDay=" + firstDay + "&firstCount=" + firstCount
						+ "&lastDay=" + lastDay + "&lastCount=" + lastCount;
