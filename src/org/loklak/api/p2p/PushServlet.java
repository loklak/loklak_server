...
public class PushServlet extends HttpServlet {
	 ...
	 * @param timeline
	 * @param hoststubs
	 *            a list of host stubs, i.e. ["http://remoteserver.eu"]
	 * @param peerMessage
	 *            if message is send to a peer
	...
	public static boolean push(String[] hoststubs, Timeline timeline, boolean peerMessage) {
		...
		try {
			...
			for (String hoststub : hoststubs) {
				...
				post.put("data", UTF8.getBytes(data)); // optionally implement a
														// gzipped form here
				ClientConnection connection = null;
				try {
					connection = new ClientConnection(hoststub + "/api/push.json", post,
							!"peers".equals(DAO.getConfig("httpsclient.trustselfsignedcerts", "peers")));
	...
	/*
	 * There are the following sources for data, pushed or retrieved: - twitter
	 * (scraped self) - remote (pushed by a remote peer, could be faked by a
	 * user) - user (pushed by a user)
	 */
	...
	/*
	 * call this i.e. with curl -i -F callback=p -F data=@tweets.json
	 * http://localhost:9000/api/push.json
	 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (data == null || data.length == 0) {
			response.sendError(400,
					"your request does not contain a data object. The data object should contain data to be pushed. The format of the data object is JSON; it is exactly the same as the JSON search result");
			return;
		}
		...
		// in case that a peer submitted their peer id, we return also some
		// statistics for that peer
		...
		if (remoteHashFromPeerId)
			json.put("contribution_message_count", messages_from_client);
		...
		DAO.log(request.getServletPath() + " -> records = " + recordCount +
		// ", new = " + newCount +
		// ", known = " + knownCount +
				", from host hash " + remoteHash + (query == null ? "" : " for query=" + query) + ", timeParsing = "
				+ (timeParsing - timeStart) + ", timeTimelineStorage = " + (timeTimelineStorage - timeParsing)
				+ ", timeQueryStorage = " + (timeQueryStorage - timeTimelineStorage) + ", timeResponse = "
				+ (timeResponse - timeQueryStorage) + ", total time = " + (timeResponse - timeStart));
