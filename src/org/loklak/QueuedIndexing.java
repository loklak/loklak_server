...
public class QueuedIndexing extends Thread {
	...
	private static BlockingQueue<DAO.MessageWrapper> messageQueue = new ArrayBlockingQueue<DAO.MessageWrapper>(
		...
		loop: while (this.shallRun)
			try {
				...
				if (messageQueue.isEmpty() || !DAO.wait_ready(1000)) {
					// in case that the queue is empty, try to fill it with
					// previously pushed content
					// List<Map<String, Object>> shard =
					// this.jsonBufferHandler.getBufferShard();
				...
				pollloop: while ((mw = messageQueue.poll()) != null) {
					...
					// in case that the message queue is too large, dump the
					// queue into a file here
					// to make room that clients can continue to push without
					// blocking
					/*
					 * if (messageQueue.size() > bufferLimit) {
					 * this.jsonBufferHandler.buffer(mw.t.getCreatedAt(),
					 * mw.t.toMap(mw.u, false, Integer.MAX_VALUE, "")); continue
					 * pollloop; }
					 */
					// if there is time enough to finish this, continue to write
					// into the index

					mw.t.enrich(); // we enrich here again because the remote
									// peer may have done this with an outdated
	...
	private void dumpbulk(List<DAO.MessageWrapper> bulk, AtomicInteger candMessages, AtomicInteger knownMessagesCache) {
		...
		DAO.log("dumped timelines: " + candMessages + " new " + knownMessagesCache + " known from cache, storage time: "
				+ (dumpfinish - dumpstart) + " ms, remaining messages: " + messageQueue.size());
		candMessages.set(0);
	...
	public static void addScheduler(Timeline tl, final boolean dump) {
		...
		for (MessageEntry me : tl)
			addScheduler(me, tl.getUser(me), dump);
