...
public class YoutubeScraper {
	...
	public static JSONObject parseVideo(final BufferedReader br) throws IOException {
		...
		while ((input = br.readLine()) != null)
			try {
				input = input.trim();
				// System.out.println(input); // uncomment temporary to debug or
				// add new fields
				int p;
				...
				if (parse_license) {
					...
					if ((p = input.indexOf("watch-view-count")) >= 0) {
						...
						if (viewcount_string.length() == 0)
							continue;
						long viewcount = 0;
						// if there are no views, there may be a string saying
						// "No". But this is done in all languages, so we just
	...
	private static String parseTag(String line, int start) {
		...
		if (p < 0)
			return null;
		int c = 1; // we count the number of open tags and stop if the number is
					// zero. We already passed the first tag which is c = 1
