...
public class RSSReader {	
	 ...
	 * For Open Source News, pass the URL: https://opensource.com/user/14486/feed
	 * in the function to obtain a corresponding JSON
	 */
	@SuppressWarnings({ "unchecked", "static-access" })
	public static JSONObject readRSS(String url){
	        ...
	        for (SyndEntry entry : (List<SyndEntry>)feed.getEntries()) {
	        	...
	        	result[i][0] = ( (entry.getTitle() == null) ? ("null") : entry.getTitle().toString() );
	        	result[i][1] = ( (entry.getLink() == null) ? ("null") : entry.getLink().toString() );
	        	result[i][2] = ( (entry.getUri() == null) ? ("null") : entry.getUri().toString() );
	        	result[i][3] = ( (Integer.toString(entry.hashCode()) == null) ? ("null") : Integer.toString(entry.hashCode()) ); 
	        	result[i][4] = ( (entry.getPublishedDate() == null) ? ("null") : entry.getPublishedDate().toString() );
	        	result[i][5] = ( (entry.getUpdatedDate() == null) ? ("null") : entry.getUpdatedDate().toString() );
	        	result[i][6] = ( (entry.getDescription() == null ) ? ("null") : entry.getDescription().toString() );
	    ...
	    rssFeed.put( url + " RSS Feed ", jsonArray);
