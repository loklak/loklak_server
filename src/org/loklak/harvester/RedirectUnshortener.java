...
public class RedirectUnshortener {
	...
	private final static String[] workingHosts = new String[] { "bbc.in", "fb.me", "wp.me", "j.mp", "t.co", "bit.ly",
			"ift.tt", "goo.gl", "tinyurl.com", "ow.ly", "tiny.cc", "bit.do", "amzn.to", "tmblr.co", "tumblr.com",
			"www.tumblr.com" };

	private final static String[] untestedHosts = new String[] { "is.gd", "ta.gd", "cli.gs", "sURL.co.uk", "y.ahoo.it",
			"yi.tl", "su.pr", "Fwd4.Me", "budurl.com", "snipurl.com", "igg.me", "twiza.ru" };
		...
		try {
			...
			while (isApplicable(urlstring) && termination-- > 0) {
				...
				if (unshortened.equals(urlstring))
					return urlstring;
				urlstring = unshortened; // recursive apply unshortener because
											// some unshortener are applied
											// several times
			}
			// DAO.log("UNSHORTENED in " + (System.currentTimeMillis() - start)
			// + " milliseconds: " + urlstring);
			return urlstring;
		} catch (IOException e) {
	 ...
	 * Surprisingly it's much slower, but some redirects cannot be discovered
	 * with the other method, but with this one.
	 * 
	 * @param urlstring
	 * @return
	 * @throws IOException
	...
	private static String getRedirect(String urlstring) throws IOException {
		...
		if (line != null && line.contains("301")) {
			...
			// skip most of the next lines, but one should start with
			// "Location:"
	...
	public static void main(String[] args) {
		String[] test = new String[] { "http://tmblr.co/Z6YPNx1jL1hHK", "http://dlvr.it/8kTDbJ",
				"http://fb.me/4lcXZsyyO", "http://wp.me/p4yQu6-za0", "http://j.mp/1vfXKr0", "http://t.co/E3w7s2qdBT",
				"http://bit.ly/1h9gTTT", "http://ift.tt/1I2O4pF", "http://goo.gl/R9CVuz", "http://tinyurl.com/pcp7fu4",
				"http://ow.ly/JtOPA", "http://tiny.cc/60ohux", "http://bit.do/ZwrT", "http://amzn.to/MO51If" };
		for (String t : test) {
			try {
				long start = System.currentTimeMillis();
	...
	/**
	 * does not work: https://tr.im/v31Rf http://dlvr.it/8htd6W // works on
