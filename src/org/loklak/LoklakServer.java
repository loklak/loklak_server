...
public class LoklakServer {
	...
	private static void setServerHandler(File dataFile) {
		...
		if (redirect || auth) {
			...
			if (auth)
				LoklakServer.server.addBean(loginService);
		...
		MultipartConfigElement multipartConfig = new MultipartConfigElement(tmp.getAbsolutePath(),
				multipartConfigDefault.getMaxFileSize(), multipartConfigDefault.getMaxRequestSize(), 1024 * 1024); // reduce
																													// IO
																													// using
																													// a
																													// non-zero
		...
		Class<? extends Servlet>[] services = new Class[] { AppsService.class, AuthorizationDemo.class,
				HelloService.class, ConsoleService.class, SignUpService.class, LoginService.class,
				PasswordRecoveryService.class, TopMenuService.class, ClientHandshake.class, PasswordResetService.class,
		...
		rewriteHandler.setOriginalPathAttribute("originalPath"); // the
																	// attribute
																	// name
																	// where the
																	// original
