...
public class Crawler {
	...
	public static boolean stack(String query, int depth, boolean followHashtags, boolean followUsers, boolean atfront) {
		...
		if (pending.size() == 0) {
			...
			long timeout = System.currentTimeMillis() - DateParser.HOUR_MILLIS; // 1
																				// hour:
																				// a
																				// user
																				// rarely
	...
	public static JSONObject toJSON() {
		...
		Set<String> processedQueries = new HashSet<String>();
		processedQueries.addAll(stacked.keySet());
		for (Term t : pending) {
			pendingQueries.add(t.query);
			processedQueries.remove(t.query);
		}
