...
public class Classifier {
	...
	public enum Category {
		joy, trust, fear, surprise, sadness, disgust, anger, anticipation, swear, sex, leet, troll, english, german, french, spanish, NONE;
	}
	...
	public enum Context {
		...
		emotion(new Category[] { Category.joy, Category.trust, Category.fear, Category.surprise, Category.sadness,
				Category.disgust, Category.anger, Category.anticipation }), profanity(
						new Category[] { Category.swear, Category.sex, Category.leet, Category.troll }), language(
								new Category[] { Category.english, Category.german, Category.french,
										Category.spanish });
		...
		public void init(final int capacity) {
			...
			for (Category f : this.categories.keySet()) {
				...
				for (String key : keys.toLowerCase().split(","))
					keyset.add(key);
				this.categories.put(f, keyset);
			}
			// consistency check of categories: identify words appearing not in
			// one category only
			...
			for (Map.Entry<Category, Set<String>> c0 : this.categories.entrySet()) {
				for (String key : c0.getValue()) {
					doublecheck: for (Map.Entry<Category, Set<String>> c1 : this.categories.entrySet()) {
						if (c1.getKey().equals(c0.getKey()))
							continue doublecheck;
						if (c1.getValue().contains(key)) {
							inconsistentWords.add(key);
							break doublecheck;
						}
					}
				}
		...
		public Set<String> vocabulary() {
			...
			for (Set<String> v0 : this.categories.values())
				v.addAll(v0);
			return v;
		}
	...
	public static Map<Context, Classification<String, Category>> classify(String phrase) {
		...
		for (Context c : Context.values()) {
			...
			if (classification == null)
				return null;
			if (classification.getProbability() == 0.0)
				return null;
	...
	public static void init(int maxsize, int initsize) {
		...
		/*
		 * // ensure consistency throughout the contexts: remove words which
		 * could confuse the bayesian filter for (Context c: Context.values()) {
		 * Set<String> voc = c.vocabulary(); for (Context c0: Context.values())
		 * { if (c0.equals(c)) continue; for (String key: voc) c0.forget(key); }
		 * }
		 */
		...
		if (DAO.countLocalMessages(-1) > 0) {
			...
			DAO.SearchLocalMessages testset = new DAO.SearchLocalMessages("", Timeline.Order.CREATED_AT, 0, initsize,
					0);
			...
			for (Context c : Context.values()) {
				...
				for (MessageEntry m : tl) {
					...
					count++;
					if (count % 100 == 0)
						DAO.log("Classifier: performed " + count + " learn steps");
				}
			}
		}
		/*
		 * for (MessageEntry m: tl) { System.out.println(m.getText());
		 * System.out.print("  -> "); Map<Context, Classification<String,
		 * Category>> classification = classify(m.getText()); for
		 * (Map.Entry<Context, Classification<String, Category>> c:
		 * classification.entrySet()) { System.out.print(c.getKey().name() +
		 * " = " + c.getValue().getCategory() + "[" +
		 * c.getValue().getProbability() + "]" + "  "); } System.out.println();
		 * }
		 */
	}
