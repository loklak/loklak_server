...
/**
 * A Query is a recording of a search result based on the query. THIS IS NOT
 * RECORDED TO TRACK USER ACTIONS, THIS IS USED TO RE-SEARCH A QUERY
 * INDEFINITELY! Each query will be stored in elasticsearch and retrieved by the
 * caretaker process in order of the retrieval_next field. That date is
 * calculated based on the number of search results in the last time; the
 * retrieval_next is estimated based on the time interval of all tweets in the
 * search results of the last query.
 * 
 * Privacy is important: TO ALL COMMITTERS: please do not add any
 * user-identification details to the data structures to protect the privacy of
 * the users; TO CODE EVALUATORS: please look for yourself that this code does
 * not contain any user-related information (like IP, user agent etc.).
 */
public class QueryEntry extends AbstractObjectEntry implements ObjectEntry {
	...
	private final static int RETRIEVAL_CONSTANT = 20; // the number of messages
														// that we get with each
														// retrieval at maximum

	protected String query; // the query in the exact way as the user typed it
							// in
	protected int query_length; // the length in the query, number of characters
	public SourceType source_type; // the (external) retrieval system where that
									// query was submitted
	protected int timezoneOffset; // the timezone offset of the user
	protected Date query_first; // the date when this query was submitted by the
								// user the first time
	protected Date query_last; // the date when this query was submitted by the
								// user the last time
	protected Date retrieval_last; // the last time when this query was
									// submitted to the external system
	protected Date retrieval_next; // the estimated next time when the query
									// should be submitted to get all messages
	protected Date expected_next; // the estimated next time when one single
									// message will appear
	protected int query_count; // the number of queries by the user of that
								// query done so far
	protected int retrieval_count; // the number of retrievals of that query
									// done so far to the external system
	protected long message_period; // the estimated period length between two
									// messages
	protected int messages_per_day; // a message frequency based on the last
									// query
	protected long score_retrieval; // score for the retrieval order
	protected long score_suggest; // score for the suggest order

	/**
	 * This initializer can only be used for first-time creation of a query
	 * track.
	 * 
	 * @param query
	 ...
	 */
	public QueryEntry(final String query, final int timezoneOffset, final long message_period,
			final SourceType source_type, final boolean byUserQuery) {
		this.query = query;
	...
	public void init(JSONObject json) throws IllegalArgumentException, JSONException {
		...
		if (source_type_string == null)
			source_type_string = SourceType.TWITTER.toString();
	 ...
	 * @param message_period
	 * @param byUserQuery
	 *            is true, if the query was submitted by the user; false if the
	 *            query was submitted by an automatic system
	 */
	public void update(final long message_period, final boolean byUserQuery) {
		// message_period may have the value Long.MAX_VALUE if search requests
		// have been empty and a message period cannot be computed
		...
		long new_message_period = message_period; // can be Long.MAX_VALUE if
													// less than 2 messages are
													// in timeline!
		int new_messages_per_day = (int) (DAY_MILLIS / new_message_period); // this
																			// is
																			// an
																			// interpolation
																			// based
		...
		if (new_message_period == Long.MAX_VALUE || new_messages_per_day == 0) {
			...
			this.message_period = this.message_period == 0 ? new_message_period
					: (this.message_period + new_message_period) / 2;
		}
		...
		long strategic_period = // if the period is far below the minimum, we
								// apply a penalty
	...
	/**
	 * A 'blind' update can be done if the user submits a query but there are
	 * rules which prevent that the target system is queried as well. Then the
	 * query result is calculated using the already stored messages. To reflect
	 * this, only the query-related attributes are changed.
	 */
	public void update() {
		this.query_count++;
	...
	private final static Pattern tokenizerPattern = Pattern.compile("([^\"]\\S*|\".+?\")\\s*"); // tokenizes
																								// Strings
																								// into
																								// terms
																								// respecting
	...
	private static enum Constraint {
		image("images"), audio("audio"), video("videos"), place("place_name"), location("location_point"), link(
				"links"), mention("mentions"), source_type("source_type"), hashtag("hashtags"), emotion(
						"classifier_emotion"), profanity("classifier_profanity"), language("classifier_language");
	...
	public static class Tokens {
		...
		public Tokens(final String q) {
			...
			if (this.constraints_negative.remove("about"))
			...
			// fix common mistake using hashtags in combination with their words
			// without hashtag
			for (String h : hashtags) {
				...
				if (p >= 0)
					this.raw = this.raw.substring(0, p) + "#" + h + " OR " + h
							+ this.raw.substring(p + h.length() * 2 + 2);
			}
			...
			bboxsearch: for (String cs : this.constraints_positive) {
				if (cs.startsWith(Constraint.location.name() + "=")) {
					...
					if (coord.length == 4) {
						...
						for (int i = 0; i < 4; i++)
							this.bbox[i] = Double.parseDouble(coord[i]);
						break bboxsearch;
					}
				}
			}
		...
		public String translate4scraper() {
			...
			// find largest city around to compute a 'near:' operator for
			// twitter
			...
			GeoLocation largestCity = DAO.geoNames.getLargestCity(lon_west + lon_border, lat_south + lat_border,
					lon_east - lon_border, lat_north - lat_border);
			if (largestCity == null)
				largestCity = DAO.geoNames.getLargestCity(lon_west, lat_south, lon_east, lat_north);
			if (largestCity == null)
				largestCity = DAO.geoNames.cityNear((lat_north + lat_south) / 2.0, (lon_east + lon_west) / 2.0);
			String q = this.raw + " near:\"" + largestCity.getNames().iterator().next() + "\" within:"
					+ ((int) (within_km / 1.609344)) + "mi"; // stupid imperial
																// units are
																// stupid
			return q;
		}
	...
	public static Timeline applyConstraint(Timeline tl0, Tokens tokens, boolean applyLocationConstraint) {
		if (tokens.constraints_positive.size() == 0 && tokens.constraints_negative.size() == 0
				&& tokens.modifier.size() == 0)
			return tl0;
		...
		messageloop: for (MessageEntry message : tl0) {
			...
			if (tokens.modifier.containsKey("from")) {
				for (String screen_name : tokens.modifier.get("from")) {
					if (!message.getScreenName().equals(screen_name))
			...
			if (applyLocationConstraint && tokens.bbox != null) {
				if (message.location_point == null || message.location_point.length < 2)
					continue messageloop; // longitude, latitude
				if (message.location_point[0] < tokens.bbox[0] || message.location_point[0] > tokens.bbox[2] || // double[]{lon_west,lat_south,lon_east,lat_north}
						message.location_point[1] > tokens.bbox[1] || message.location_point[1] < tokens.bbox[3])
					continue messageloop;
			}

			// check constraints
			if (tokens.constraints_positive.contains("image") && message.getImages().size() == 0)
				continue;
			if (tokens.constraints_negative.contains("image") && message.getImages().size() != 0)
				continue;
			if (tokens.constraints_positive.contains("place") && message.getPlaceName().length() == 0)
				continue;
			if (tokens.constraints_negative.contains("place") && message.getPlaceName().length() != 0)
				continue;
			if (tokens.constraints_positive.contains("location")
					&& (message.getLocationPoint() == null || message.getPlaceContext() != tokens.place_context))
				continue;
			if (tokens.constraints_negative.contains("location") && message.getLocationPoint() != null)
				continue;
			if (tokens.constraints_positive.contains("link") && message.getLinks().length == 0)
				continue;
			if (tokens.constraints_negative.contains("link") && message.getLinks().length != 0)
				continue;
			if (tokens.constraints_positive.contains("mention") && message.getMentions().length == 0)
				continue;
			if (tokens.constraints_negative.contains("mention") && message.getMentions().length != 0)
				continue;
			if (tokens.constraints_positive.contains("hashtag") && message.getHashtags().length == 0)
				continue;
			if (tokens.constraints_negative.contains("hashtag") && message.getHashtags().length != 0)
				continue;
			for (Classifier.Context context : Classifier.Context.values()) {
				if (tokens.constraints_positive.contains(context.name()) && message.getClassifier(context) == null)
					continue messageloop;
				if (tokens.constraints_negative.contains(context.name()) && message.getClassifier(context) != null)
					continue messageloop;
			}
			...
			constraintCheck: for (String cs : tokens.constraints_positive) {
				if (cs.startsWith(Constraint.location.name() + "=")) {
					if (message.getLocationPoint() == null)
						continue messageloop;
					if (message.getPlaceContext() != tokens.place_context)
						continue messageloop;
					String params = cs.substring(Constraint.location.name().length() + 1);
					String[] coord = params.split(",");
					if (coord.length == 4) {
	...
	private final static Pattern term4ORPattern = Pattern.compile("(?:^| )(\\S*(?: OR \\S*)+)(?: |$)"); // Pattern.compile("(^\\s*(?:
																										// OR
																										// ^\\s*+)+)");

	private static List<String> splitIntoORGroups(String q) {
		// detect usage of OR junctor usage. Right now we cannot have mixed AND
		// and OR usage. Thats a hack right now
	...
	public static class ElasticsearchQuery {
		...
		private QueryBuilder preparse(String q, int timezoneOffset) {
			...
			q = q.replaceAll(" AND ", " "); // AND is default
			List<String> terms = splitIntoORGroups(q); // OR binds stronger than
														// AND
			if (terms.size() == 0)
				return QueryBuilders.constantScoreQuery(QueryBuilders.matchAllQuery());
		...
		private QueryBuilder parse(String q, int timezoneOffset) {
			// detect usage of OR ORconnective usage. Because of the preparse
			// step we will have only OR or only AND here.
			q = q.replaceAll(" AND ", " "); // AND is default
			boolean ORconnective = q.indexOf(" OR ") >= 0;
			q = q.replaceAll(" OR ", " "); // if we know that all terms are OR,
											// we remove that and apply it later
			...
			// since:2015-04-01 until:2015-04-03 - tweets within given time
			// range
			// additional constraints:
			// /image /audio /video /place - restrict to tweets which have
			// attached images, audio, video or place
			...
			for (String t : qe) {
				...
				if (t.startsWith("@")) {
					...
					// patch characters that will confuse elasticsearch or have
					// a different meaning
					...
					if ((t.charAt(0) == '"' && t.charAt(t.length() - 1) == '"')
						...
						// this must be handled like a quoted string without the
						// minus
						t = t.replaceAll("-", " ");
						if (negative)
							text_negative_filter.add(t);
			...
			if (modifier.containsKey("to"))
				users_positive.addAll(modifier.get("to"));
			if (modifier.containsKey("-to"))
				users_negative.addAll(modifier.get("-to"));

			// special constraints
			boolean constraint_about = constraints_positive.remove("about");
			if (constraints_negative.remove("about"))
				constraint_about = false;
			...
			for (String text : text_negative_match) {
				// negation of terms in disjunctions would cause to retrieve
				// almost all documents
				// this cannot be the requirement of the user. It may be valid
				// in conjunctions, but not in disjunctions
			...
			for (String user : users_negative)
				nops.add(QueryBuilders.constantScoreQuery(QueryBuilders.termQuery("mentions", user)));
			...
			for (String hashtag : hashtags_negative)
				nops.add(QueryBuilders.constantScoreQuery(QueryBuilders.termQuery("hashtags", hashtag.toLowerCase())));
				...
				for (String screen_name : modifier.get("from")) {
					if (screen_name.indexOf(',') < 0) {
						...
						for (String name : screen_names)
							disjunction.should(
									QueryBuilders.constantScoreQuery(QueryBuilders.termQuery("screen_name", name)));
			...
			if (modifier.containsKey("-from")) {
				for (String screen_name : modifier.get("-from")) {
					if (screen_name.indexOf(',') < 0) {
						...
						String[] screen_names = screen_name.split(",");
						for (String name : screen_names)
							nops.add(QueryBuilders.constantScoreQuery(QueryBuilders.termQuery("screen_name", name)));
					}
				}
			}
			if (modifier.containsKey("near")) {
				...
				if (loc == null) {
					...
					ops.add(QueryBuilders.boolQuery().filter(nearquery).filter(QueryBuilders
							.constantScoreQuery(QueryBuilders.matchQuery("place_context", PlaceContext.FROM.name()))));
				} else {
					filters.add(QueryBuilders.geoDistanceQuery("location_point")
							.distance(100.0, DistanceUnit.KILOMETERS).lat(loc.lat()).lon(loc.lon()));
				}
			}
			if (modifier.containsKey("since"))
				try {
					...
					if (modifier.containsKey("until")) {
						...
						if (until.get(Calendar.HOUR) == 0 && until.get(Calendar.MINUTE) == 0) {
							// until must be the day which is included in
							// results.
							// To get the result within the same day, we must
							// add one day.
				...
				try {
					...
					if (until.get(Calendar.HOUR) == 0 && until.get(Calendar.MINUTE) == 0) {
						...
						// To get the result within the same day, we must add
						// one day.
						until.add(Calendar.DATE, 1);
					}
					this.until = until.getTime();
			...
			if (ops.size() == 1 && nops.size() == 0)
				...
				for (QueryBuilder qb : ops) {
					if (ORconnective)
						((BoolQueryBuilder) bquery).should(qb);
					else
						((BoolQueryBuilder) bquery).filter(qb);
				}
				if (ORconnective)
			...
			for (String text : text_negative_filter)
				filters.add(QueryBuilders.boolQuery()
						.mustNot(QueryBuilders.constantScoreQuery(QueryBuilders.termsQuery("text", text))));
			...
			if (constraints_positive.contains("location")) {
				filters.add(QueryBuilders.constantScoreQuery(QueryBuilders.termsQuery("place_context",
						(constraint_about ? PlaceContext.ABOUT : PlaceContext.FROM).name())));
			}

			// special treatment of location constraints of the form
			// /location=lon-west,lat-south,lon-east,lat-north i.e.
			// /location=8.58,50.178,8.59,50.181
			// source_type constraint of the form /source_type=FOSSASIA_API ->
			// search exact term (source_type must exists in SourceType enum)
				...
				if (cs.startsWith(Constraint.location.name() + "=")) {
					...
					if (coord.length == 1) {
						filters.add(QueryBuilders
								.constantScoreQuery(QueryBuilders.termsQuery("location_source", coord[0])));
					} else if (coord.length == 2) {
						double lon = Double.parseDouble(coord[0]);
						double lat = Double.parseDouble(coord[1]);
						// ugly way to search exact geo_point : using
						// geoboundingboxfilter, with two identical bounding
						// points
						// geoshape filter can search for exact point shape but
						// it can't be performed on geo_point field
						filters.add(QueryBuilders.geoBoundingBoxQuery("location_point").topLeft(lat, lon)
								.bottomRight(lat, lon));
						...
						filters.add(QueryBuilders
								.constantScoreQuery(QueryBuilders.termsQuery("place_context", context.name())));
						filters.add(QueryBuilders.geoBoundingBoxQuery("location_point").topLeft(lat_north, lon_west)
								.bottomRight(lat_south, lon_east));
						if (coord.length == 5)
							filters.add(QueryBuilders
									.constantScoreQuery(QueryBuilders.termsQuery("location_source", coord[4])));
					}
			...
			for (String cs : constraints_negative) {
				if (cs.startsWith(Constraint.source_type.name() + "=")) {
					...
					if (SourceType.isValid(regexp)) {
						filters.add(QueryBuilders.boolQuery()
								.mustNot(QueryBuilders.constantScoreQuery(QueryBuilders.termQuery("_type", regexp))));
					}
				}
			}
			...
			QueryBuilder cquery = filters.size() == 0 ? bquery
					: QueryBuilders.boolQuery().filter(bquery).filter(queryFilter);
			return cquery;
		}
	}
