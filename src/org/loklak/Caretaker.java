...
public class Caretaker extends Thread {
	...
	public void run() {
		...
		boolean busy = false;
		// work loop
		beat: while (this.shallRun)
			try {
				// check upgrate time
				if (System.currentTimeMillis() > upgradeTime) {
					// increase the upgrade time to prevent that the peer runs
					// amok (re-tries the attempt all the time) when upgrade
				...
				// clear caches
				if (SuggestServlet.cache.size() > 100)
					SuggestServlet.cache.clear();

				// sleep a bit to prevent that the DoS limit fires at backend
				// server
				try {
					Thread.sleep(busy ? 1000 : 5000);
				...
				// DAO.log("connection pool: " +
				// ClientConnection.cm.getTotalStats().toString());
				...
				if (tl != null && tl.size() > 0 && remote.length > 0) {
					...
					if (success) {
						DAO.log("success pushing " + tl.size() + " messages to backend in 1st attempt in "
								+ (System.currentTimeMillis() - start) + " ms");
					}
					if (!success) {
						// we should try again.. but not an infinite number
						// because then
						// our timeline in RAM would fill up our RAM creating a
						// memory leak
						retrylook: for (int retry = 0; retry < 5; retry++) {
							// give back-end time to recover
							try {
								Thread.sleep(3000 + retry * 3000);
							} catch (InterruptedException e) {
							}
							DAO.log("trying to push (again) " + tl.size() + " messages to backend, attempt #" + retry
									+ 1 + "/5");
							start = System.currentTimeMillis();
							if (PushServlet.push(remote, tl)) {
								DAO.log("success pushing " + tl.size() + " messages to backend in " + (retry + 2)
										+ ". attempt in " + (System.currentTimeMillis() - start) + " ms");
								success = true;
								break retrylook;
							}
						}
				...
				if (DAO.getConfig("retrieval.forbackend.enabled", false) && DAO.getConfig("backend.push.enabled", false)
					...
					int retrieval_forbackend_sleep_randomoffset = (int) DAO
							.getConfig("retrieval.forbackend.sleep.randomoffset", 100);
					hloop: for (int i = 0; i < retrieval_forbackend_loops; i++) {
						...
						for (int j = 0; j < retrieval_forbackend_concurrency; j++) {
							...
							rts[j].start();
							try {
								Thread.sleep(retrieval_forbackend_sleep_base
										+ random.nextInt(retrieval_forbackend_sleep_randomoffset));
							} catch (InterruptedException e) {
							}
						}
						for (Thread t : rts)
				...
				for (int i = 0; i < 10; i++) {
					if (Crawler.process() == 0)
						break; // this may produce tweets for the timeline push
					try {
				...
				if (DAO.getConfig("retrieval.queries.enabled", false)) {
					// execute some queries again: look out in the suggest
					// database for queries with outdated due-time in field
					// retrieval_next
					List<QueryEntry> queryList = DAO.SearchLocalQueries("", 10, "retrieval_next", "date", SortOrder.ASC,
					...
					for (QueryEntry qe : queryList) {
						...
						Timeline t = DAO.scrapeTwitter(null, qe.getQuery(), Timeline.Order.CREATED_AT,
								qe.getTimezoneOffset(), false, 10000, true);
				...
				if (ids != null && DAO.getConfig("retrieval.user.enabled", false)
						&& TwitterAPI.getAppTwitterFactory() != null) {
					try {
						TwitterAPI.getScreenName(ids, 10000, false);
					} catch (IOException | TwitterException e) {
						for (Number n : ids)
							DAO.announceNewUserId(n); // push back unread values
				...
				// heal the latency to give peers with out-dated information a
				// new chance
	...
	public static void transmitMessage(final MessageEntry tweet, final UserEntry user) {
		if (DAO.getConfig("backend", new String[0], ",").length <= 0)
			return;
		if (!DAO.getConfig("backend.push.enabled", false))
			return;
		Timeline tl = pushToBackendTimeline.poll();
		if (tl == null)
			tl = new Timeline(Timeline.Order.CREATED_AT);
		tl.add(tweet, user);
		pushToBackendTimeline.add(tl);
	}

	/**
	 * if the given list of timelines contain at least the wanted minimum size
	 * of messages, they are flushed from the queue and combined into a new
	 * timeline
	 * 
	 * @param dumptl
	 * @param order
