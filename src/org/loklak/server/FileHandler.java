...
public class FileHandler extends ResourceHandler implements Handler {
	 ...
	 *            the time each file shall stay in the cache
	 */
	...
	public void handle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response)
		...
		// use the ResourceHandler to handle the request. This method calls
		// doResponseHeaders internally
	...
	public static void setCaching(final HttpServletResponse response, final int expiresSeconds) {
		if (response instanceof org.eclipse.jetty.server.Response) {
			...
			// remove the last-modified field since caching otherwise does not
			// work
			/*
			 * https://www.ietf.org/rfc/rfc2616.txt "if the response does have a
			 * Last-Modified time, the heuristic expiration value SHOULD be no
			 * more than some fraction of the interval since that time. A
			 * typical setting of this fraction might be 10%."
			 */
			fields.remove(HttpHeader.LAST_MODIFIED); // if this field is
														// present, the
														// reload-time is a 10%
														// fraction of ttl and
			...
			// cache-control: allow shared caching (i.e. proxies) and set
			// expires age for cache
			fields.put(HttpHeader.CACHE_CONTROL, "public, max-age=" + Integer.toString(expiresSeconds)); // seconds
		} else {
			response.setHeader(HttpHeader.LAST_MODIFIED.asString(), ""); // not
																			// really
																			// the
																			// best
																			// wqy
		...
		// expires: define how long the file shall stay in a cache if
		// cache-control is not used for this information
