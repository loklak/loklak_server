...
<<<<<<< HEAD
	...
	public JSONObject[] service(Query call, Authorization rights) throws APIException {
		...
		if (this.serverProtocolHostStub == null)
			return new JSONObject[] { serviceImpl(call, rights) };
...
>>>>>>> aac1787db3815d09c0c35cd0d2f43caad15ad536
		...
		// user authorization: we use the identification of the user to get the
		...
		if (authorization.getBaseUserRole().ordinal() < minimalBaseUserRole.ordinal()) {
			response.sendError(401,
					"Base user role not sufficient. Your base user role is '" + authorization.getBaseUserRole().name()
							+ "', your user role is '" + authorization.getUserRole().getDisplayName() + "'");
			return;
		}

		// user accounting: we maintain static and persistent user data; we
		// again search the accounts using the usder identity string
		// JSONObject accounting_persistent_obj =
		// DAO.accounting_persistent.has(user_id) ?
		// DAO.accounting_persistent.getJSONObject(anon_id) :
		// DAO.accounting_persistent.put(user_id, new
		// JSONObject()).getJSONObject(user_id);
...
<<<<<<< HEAD
	 ...
	 * 
	 * @return user identity if some login is active, anonymous identity
	 *         otherwise
	 */
...
>>>>>>> aac1787db3815d09c0c35cd0d2f43caad15ad536
		...
		} else if (request.getSession().getAttribute("identity") != null) { // if
																			// identity
																			// is
																			// registered
																			// for
		...
		} else if (request.getParameter("login") != null && request.getParameter("password") != null) { // check
																										// if
																										// login
																										// parameters
																										// are
				...
				if (authentication.has("activated") && authentication.getBoolean("activated")) {
					...
					if (authentication.has("passwordHash") && authentication.has("salt")) {
						...
						if (getHash(password, salt).equals(passwordHash)) {
							...
							// only create a cookie or session if requested (by
							// login page)
							if ("true".equals(request.getParameter("request_cookie"))) {
								...
								ClientCredential cookieCredential = new ClientCredential(ClientCredential.Type.cookie,
										loginToken);
							...
							Log.getLog().info("login for user: " + credential.getName() + " via passwd from host: "
									+ request.getRemoteHost());

							return identity;
						}
						Log.getLog().info("Invalid login try for user: " + credential.getName()
								+ " via passwd from host: " + request.getRemoteHost());
						throw new APIException(422, "Invalid credentials");
					}
					Log.getLog().info("Invalid login try for user: " + credential.getName() + " from host: "
							+ request.getRemoteHost() + " : password or salt missing in database");
					throw new APIException(422, "Invalid credentials");
				}
				Log.getLog().info("Invalid login try for user: " + credential.getName() + " from host: "
						+ request.getRemoteHost() + " : user not activated yet");
				throw new APIException(422, "User not yet activated");
			} else {
				authentication.delete();
				Log.getLog().info("Invalid login try for unknown user: " + credential.getName()
						+ " via passwd from host: " + request.getRemoteHost());
				throw new APIException(422, "Invalid credentials");
			}
			...
			if (authentication.getIdentity() != null) {
				...
				Log.getLog().info("Invalid login try for user: " + identity.getName()
						+ " via expired access token from host: " + request.getRemoteHost());
				authentication.delete();
	...
	private ClientIdentity getAnonymousIdentity(HttpServletRequest request) {
		...
		if (authentication.getIdentity() == null)
			authentication.setIdentity(new ClientIdentity(credential.toString()));
