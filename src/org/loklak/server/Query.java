...
public class Query {
	...
	public Query(final HttpServletRequest request) {
		...
		if (XRealIP != null && XRealIP.length() > 0)
			clientHost = XRealIP; // get IP through nginx config
		...
		this.track.setTimeSinceLastAccess(
				this.track.getDate().getTime() - RemoteAccess.latestVisit(this.track.getClassName(), clientHost));
		// System.out.println("*** this.time_since_last_access = " +
		// this.time_since_last_access);
		this.track.setDoSBlackout(LoklakServer.blacklistedHosts.contains(clientHost) || (!this.track.isLocalhostAccess()
				&& (this.track.getTimeSinceLastAccess() < DAO.getConfig("DoS.blackout", 100))));
		this.track.setDoSServicereduction(!this.track.isLocalhostAccess()
				&& (this.track.getTimeSinceLastAccess() < DAO.getConfig("DoS.servicereduction", 1000)));
	}
	...
	public void initPOST(final Map<String, byte[]> map) {
		...
		for (Map.Entry<String, byte[]> entry : map.entrySet())
			this.qm.put(entry.getKey(), UTF8.String(entry.getValue()));
	}
