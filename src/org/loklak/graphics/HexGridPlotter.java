...
public class HexGridPlotter extends RasterPlotter {
	...
	private int projectionX(final int x, final int y) {
		return (x + 1) * this.cellwidth12 - this.cellwidth2
				+ ((x & 1) == 0 ? ((y & 1) == 0 ? 0 : this.cellwidth2) : ((y & 1) == 0 ? this.cellwidth2 : 0));
	}
	...
	public void gridPrint(final int x, final int y, final int radius, final String messageHigh, final String messageLow,
			final int align, int intensity) {
		if (messageHigh != null && messageHigh.length() > 0)
			PrintTool.print(this, projectionX(x, y) - align * radius,
					projectionY(y) - (cellwidth2 >> 2) - ((align == 0) ? radius : 0), 0, messageHigh, align, false,
					intensity);
		if (messageLow != null && messageLow.length() > 0)
			PrintTool.print(this, projectionX(x, y) - align * radius,
					projectionY(y) + (cellwidth2 >> 2) + 4 + ((align == 0) ? radius : 0), 0, messageLow, align, false,
					intensity);
	}
	...
	public void gridPrint315(final int x, final int y, final int radius, final String message315, int intensity) {
		if (message315 != null && message315.length() > 0)
			PrintTool.print(this, projectionX(x, y) + (radius >> 1), projectionY(y) + (cellwidth2 >> 2) + 4, 315,
					message315, -1, false, intensity);
	}
	...
	public void gridLine(int Ax, int Ay, final int Bx, final int By, String colorLine, final int intensityLine,
			String colorDot, final int intensityDot, int dotDist, int dotPos, int dotRadius, boolean dotFilled) {
		// we do not use Bresenham's line drawing algorithm here because we want
		// only 0, 45 and 90 degree lines
	...
	private static HexGridPlotter testImage0(int width, int height, String bgcolor, String gridcolor, String dotcolor) {
		final HexGridPlotter picture = new HexGridPlotter(width, height,
				bgcolor.equals("000000") ? DrawMode.MODE_ADD : DrawMode.MODE_SUB, bgcolor, 18);
		...
		// grid.gridLine(grid.gwidth - 2, grid.gheight - 2, grid.gwidth / 2,
		// grid.gheight / 2);
