...
/**
 * This Class implements some convenience-methods to support drawing of
 * statistical Data It is not intended to replace existing awt-funktions even if
 * it looks so This class provides some drawing methods that creates
 * transparency effects that are not available in awt.
 */
public class RasterPlotter {
	...
	public RasterPlotter(final int width, final int height, final DrawMode drawMode, final long backgroundColor) {
		...
		try {
			// we need our own frame buffer to get a very, very fast
			// transformation to png because we can omit the PixedGrabber, which
			// is up to 800 times slower
			...
			ComponentSampleModel sampleModel = new ComponentSampleModel(DataBuffer.TYPE_BYTE, width, height, 3,
					width * 3, new int[] { 0, 1, 2 });
			this.grid = Raster.createWritableRaster(sampleModel, videoBuffer, null);
			ColorModel colorModel = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), null, false,
	...
	/**
	 * draw a line using Bresenham's line drawing algorithm. The line will be
	 * plotted together with dots on it, if wanted.
	 * 
	 * @param Ax
	 * @param Ay
	 * @param Bx
	 ...
	 * @param dotFilled
	 */
	public void line(int Ax, int Ay, final int Bx, final int By, final Long colorLine, final int intensityLine,
			final Long colorDot, final int intensityDot, final int dotDist, final int dotPos, final int dotRadius,
	 ...
	 *            radius of the dot
	 * @param padding
	 *            the distance of the dot border to the end point
	 * @param lineColor
	 *            the color of the line
	 * @param dotColor
	 *            the color of the dot
	 */
	public void lineDot(final int x0, final int y0, final int x1, final int y1, final int radius, final int padding,
			final long lineColor, final long dotColor) {
		final double dx = x1 - x0; // distance of points, x component
		final double dy = y1 - y0; // distance of points, y component
		final double angle = Math.atan2(dy, dx); // the angle of the line
													// between the points
		final double d = Math.sqrt((dx * dx + dy * dy)); // the distance between
															// the points
															// (Pythagoras)
		final double ddotcenter = d - radius - padding; // distance from {x0,
														// y0} to dot center
														// near {x1, y1}
		final double ddotborder = ddotcenter - radius; // distance to point {x3,
														// y3} at border of dot
														// center at {x2, y2}
		final double xn = Math.cos(angle); // normalized vector component x
		final double yn = Math.sin(angle); // normalized vector component y
		final int x2 = x0 + ((int) (ddotcenter * xn)); // dot center, x
														// component
		final int y2 = y0 + ((int) (ddotcenter * yn)); // dot center, y
														// component
		final int x3 = x0 + ((int) (ddotborder * xn)); // dot border, x
														// component
		final int y3 = y0 + ((int) (ddotborder * yn)); // dot border, y
														// component
		setColor(lineColor);
	 ...
	 *            end point
	 * @param sidelength
	 *            the side length of the arrow tip (all 3 sides are equal)
	 * @param padding
	 *            the distance of the arrow tip to the end point
	 * @param lineColor
	 *            the color of the line
	 * @param arrowColor
	 *            the color of the arrow tip
	 */
	public void lineArrow(final int x0, final int y0, final int x1, final int y1, final int sidelength,
			final int padding, final long lineColor, final long arrowColor) {
		final double dx = x1 - x0; // distance of points, x component
		final double dy = y1 - y0; // distance of points, y component
		final double angle = Math.atan2(dy, dx); // the angle of the line
													// between the points
		final double d = Math.sqrt((dx * dx + dy * dy)); // the distance between
															// the points
															// (Pythagoras)
		final double arrowtip = d - padding; // the distance from {x0, y0} to
												// the arrow tip
		final double arrowlength = TL * sidelength; // the length of the arrow
													// (distance from base to
													// tip)
		final double arrowbase = arrowtip - arrowlength; // the distance from
															// {x0, y0} to the
															// arrow base
		final double xn = Math.cos(angle); // normalized vector component x
		final double yn = Math.sin(angle); // normalized vector component y
		final int xt = x0 + ((int) (arrowtip * xn)); // arrow tip point
														// component x
		final int yt = y0 + ((int) (arrowtip * yn)); // arrow tip point
														// component y
		...
		final double xk = sl2 * Math.cos(angle + PI2); // point at 90 degree on
														// arrow direction to
														// left side, vector
														// component x
		final double yk = sl2 * Math.sin(angle + PI2); // point at 90 degree on
														// arrow direction to
														// left side, vector
														// component y
		...
		setColor(lineColor);
		line(x0, y0, (int) xb, (int) yb, 100); // draw line from {x0, y0} to
												// arrow base
		setColor(arrowColor);
	...
	public void arc(final int x, final int y, final int innerRadius, final int outerRadius, final int fromArc,
			final int toArc) {
		for (int r = innerRadius; r <= outerRadius; r++) {
	 ...
	 *            center of circle, y
	 * @param innerRadius
	 *            inner radius of line
	 * @param outerRadius
	 *            outer radius of line
	 * @param angle
	 *            angle within the circle for the line
	 * @param in
	 *            direction, if true: inward. This is the moving direction of
	 *            dots, if dotRadius is alternated from 0 to 360
	 * @param colorLine
	 *            the color of the line
	 * @param colorDot
	 *            the color of the dot
	 * @param dotDist
	 *            the distance of two dots
	 * @param dotPos
	 *            the start position of the first dot
	 * @param dotRadius
	 *            the radius of the dot
	 * @param dotFilled
	 *            if true: dot is filled.
	 */
	public void arcLine(final int cx, final int cy, final int innerRadius, final int outerRadius, final double angle,
			final boolean in, final Long colorLine, final Long colorDot, final int dotDist, final int dotPos,
			final int dotRadius, final boolean dotFilled) {
		...
		if (in) {
			line(xo, yo, xi, yi, colorLine, 100, colorDot, 100, dotDist, dotPos, dotRadius, dotFilled);
		} else {
			line(xi, yi, xo, yo, colorLine, 100, colorDot, 100, dotDist, dotPos, dotRadius, dotFilled);
		}
	}
	 ...
	 *            radius of circle
	 * @param angle1
	 *            position of dot 1 on circle
	 * @param angle2
	 *            position of dot 2 on circle
	 * @param in
	 *            direction of dots on line; in=true means: inwards
	 * @param colorLine
	 * @param intensityLine
	 * @param colorDot
	 * @param intensityDot
	 ...
	 * @param dotFilled
	 */
	public void arcConnect(final int cx, final int cy, final int arcRadius, final double angle1, final double angle2,
			final boolean in, final Long colorLine, final int intensityLine, final Long colorDot,
			final int intensityDot, final int dotDist, final int dotPos, final int dotRadius, final boolean dotFilled,
		...
		// draw the line
		if (in) {
			line(x1, y1, x2, y2, colorLine, intensityLine, colorDot, intensityDot, dotDist, dotPos, dotRadius,
					dotFilled);
		} else {
			line(x2, y2, x1, y1, colorLine, intensityLine, colorDot, intensityDot, dotDist, dotPos, dotRadius,
					dotFilled);
		}
	...
	public void arcArc(final int cx, final int cy, final int arcRadius, final double angle, final int innerRadius,
			final int outerRadius, final int intensity) {
		final double a = PI180 * angle;
		final int x = cx + (int) (arcRadius * Math.cos(a));
		final int y = cy - (int) (arcRadius * Math.sin(a));
	...
	public void arcArc(final int cx, final int cy, final int arcRadius, final double angle, final int innerRadius,
			final int outerRadius, final int fromArc, final int toArc) {
		final double a = PI180 * angle;
		final int x = cx + (int) (arcRadius * Math.cos(a));
		final int y = cy - (int) (arcRadius * Math.sin(a));
	 ...
	 * 
	 * @param bitmap
	 *            bitmap to be inserted
	 * @param x
	 *            x-value of upper left coordinate where bitmap will be placed
	 * @param y
	 *            y-value of upper left coordinate where bitmap will be placed
	 */
	...
	/**
	 * inserts image where all pixels which have the same RGB value as the pixel
	 * at (xx, yy) are transparent
	 * 
	 * @param bitmap
	 *            bitmap to be inserted
	 * @param x
	...
	/**
	 * inserts image where all pixels that have the same RGB value as the pixel
	 * at (xx, yy) are transparent
	 * 
	 * @param bitmap
	 *            bitmap to be inserted
	 ...
	 *            y-value of upper left coordinate where bitmap will be placed
	 * @param rgb
	 *            RGB value which will be transparent
	 */
	...
	/**
	 * inserts image where all pixels that have a special RGB value pixel at
	 * (xx, yy) are transparent
	 * 
	 * @param bitmap
	 *            bitmap to be inserted
	 * @param x
	...
	public void insertBitmap(final BufferedImage bitmap, final int x, final int y, final int transRGB,
		...
		if (filter == FilterMode.FILTER_ANTIALIASING) {
			...
			// of the image, the fiter will be used. If no such pixel has been
			// found that
			// means that there either is no transparent pixel in the bitmap or
			// part
			// of the bitmap that covers part of tha image is not within the
			// borders of
			// the image (i.e. bitmap is larger than image)
			if (transX != -1) {
				filter(x - 1, y - 1, x + bitmapWidth, y + bitmapHeight, filter,
	 ...
	 * @param uly
	 *            y-value for upper left coordinate
	 * @param lrx
	 *            x-value for lower right coordinate
	 * @param lry
	 *            y-value for lower right coordinate
	 * @param rgb
	 *            color of background
	 */
	 ...
	 *            chooses filter
	 */
	private void filter(final int ulx, final int uly, final int lrx, final int lry, final FilterMode filter,
			final int bgcolor) {
		...
		for (int i = lox; i < rux + 1; i++) {
			for (int j = loy; j < ruy + 1; j++) {
				...
				// add value of pixel
				// in case filter is used for antialiasing this will only be
				// done if
	...
	public BufferedImage toIndexed() {
		...
		/*
		 * byte [] data = null; int bytesPerRow = this.getWidth()/8 +
		 * (this.getWidth()%8!=0?1:0); data = new byte[this.getHeight() *
		 * bytesPerRow]; DataBuffer db = new DataBufferByte(data, data.length);
		 * WritableRaster wr = Raster.createPackedRaster(db, this.getWidth(),
		 * this.getHeight(), 1, null); BufferedImage dest = new
		 * BufferedImage(cm, wr, false, null);
		 */

		BufferedImage dest = new BufferedImage(this.getWidth(), this.getHeight(),
				cm.getPixelSize() < 8 ? BufferedImage.TYPE_BYTE_BINARY : BufferedImage.TYPE_BYTE_INDEXED, cm);
	 ...
	 *            the storage file
	 * @param type
	 *            the file type, may be i.e. 'png' or 'gif'
	 * @throws IOException
	 */
	...
	/*
	 * The following code was transformed from a library, coded by J. David
	 * Eisenberg, version 1.5, 19 Oct 2003 (C) LGPL This code was very strongly
	 * transformed into the following very short method for an ultra-fast png
	 * generation. These changes had been made 23.10.2012 by [MC] to the
	 * original code: For the integration into YaCy this class was adopted to
	 * YaCy graphics by Michael Christen: - removed alpha encoding - removed not
	 * used code - inlined static values - inlined all methods that had been
	 * called only once - moved class objects which appear after all refactoring
	 * only within a single method into this method - removed a giant number of
	 * useless (obvious things) comments and empty lines to increase readability
	 * (!) - new order of data computation: first compute the size of compressed
	 * deflater output, then assign an exact-sized byte[] which makes resizing
	 * afterwards superfluous - after all enhancements all class objects were
	 * removed; result is just one short static method - made objects final
	 * where possible - removed the PixelGrabber call and replaced it with a
	 * call to this.frame which is just a byte[] - added more speed woodoo like
	 * a buffer around the deflater which makes this much faster
	 */
	...
	public final byte[] pngEncode(final int compressionLevel) throws IOException {
		...
		for (int row = 0; row < height; row++) {
			compBytes.write(0);
			// this replaces the whole PixelGrabber process which makes it
			// probably more than 800x faster. See
			// http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4835595
		...
		// finally write the result of the concurrent calculation into an
		// DeflaterOutputStream to compress the png
		final int nCompressed = outBytes.size();
		final byte[] pngBytes = new byte[nCompressed + 57]; // yes thats the
															// exact size, not
															// too less, not too
															// much. No resizing
															// needed.
	...
	private final static int writeInt4(final byte[] target, final int n, final int offset) {
		return writeBytes(target, new byte[] { (byte) ((n >> 24) & 0xff), (byte) ((n >> 16) & 0xff),
				(byte) ((n >> 8) & 0xff), (byte) (n & 0xff) }, offset);
	}
	...
	public static void main(final String[] args) {
		...
		/*
		 * Process p = null; try {p = Runtime.getRuntime().exec(new String[]
		 * {"/usr/bin/osascript", "-e", "open \"" + args[0] + "\""});} catch
		 * (final java.io.IOException e) {Log.logException(e);} try
		 * {p.waitFor();} catch (final InterruptedException e)
		 * {Log.logException(e);}
		 */
	}
