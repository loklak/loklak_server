...
public class AnimationGIF {
	 ...
	 * 
	 * @param loops
	 *            - number of loops for the animated images. -1 = no loops; 0 =
	 *            indefinitely loops; else: number of loops
	 */
	public AnimationGIF(int loops) {
		...
		this.writer = writerIterator.next(); // com.sun.media.imageioimpl.plugins.gif.GIFImageWriter,
	 ...
	 *            the image
	 * @param delayMillis
	 *            the frame time of the image in milliseconds
	 * @param transparencyColorIndex
	 *            the index of the transparent color, -1 if not used
	 * @throws IOException
	 */
	public void addImage(RenderedImage image, int delayMillis, int transparencyColorIndex) throws IOException {
		...
		if (this.counter == 0 && loops >= 0) {
			IIOMetadata imageMetadata2 = writer
					.getDefaultImageMetadata(ImageTypeSpecifier.createFromRenderedImage(image), iwp);
			try {
	...
	private static void setMetadata(IIOMetadata metaData, int delayMillis, int transparencyColorIndex)
		...
		if (gceNode == null)
			throw new IIOInvalidTreeException("Invalid image metadata, could not find " + gceNodeName + "node.", null,
					tree);
	 ...
	 * @param metaData
	 * @param loops
	 *            - 0 = loop continuously; 1-65535 = a specific number of loops
	 * @throws IIOInvalidTreeException
	 */
	...
	public static void main(String[] args) {
		System.setProperty("java.awt.headless", "true"); // go into headless awt
															// mode
