	...
	public void run() {
		...
		loop: while (this.shallRun)
			try {
				...
				// we start concurrent indexing threads to process the json
				// objects
							...
							try {
								...
								while ((tweet = dumpReader.take()) != JsonStreamReader.POISON_JSON_MAP) {
									try {
										...
										messageBulk
												.add(new IndexEntry<MessageEntry>(t.getIdStr(), t.getSourceType(), t));
				...
				// wait for termination of the indexing threads and do logging
				// meanwhile
